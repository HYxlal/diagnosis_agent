...
管理每个 conversation_id 的对话历史，在 Agent 调用前注入上下文。
...
class SessionRedisStore:
    ...
    def save(self, ctx: ConversationContext, ttl: int | None = None) -> None:
        ...
        if self._available and self._redis:
            try:
                ...
                self._redis.set(self._key(ctx.conversation_id), json.dumps(data, ensure_ascii=False), ex=ttl)
                return
            except Exception as e:
    ...
    def exists(self, conversation_id: str) -> bool:
        ...
        if self._available and self._redis:
            try:
                ...
                self._available = False
        return conversation_id in self._local
    ...
    def refresh_ttl(self, conversation_id: str, ttl: int) -> None:
        """刷新会话 TTL（续期）"""
        if self._available and self._redis:
            try:
                self._redis.expire(self._key(conversation_id), ttl)
            except Exception:
                pass
...
class SessionManager:
    ...
    def _save_active(self, ctx: ConversationContext) -> None:
        ...
        try:
            ...
            path = active_dir / f"{ctx.conversation_id}.json"
            with open(path, "w", encoding="utf-8") as f:
                json.dump(ctx.to_dict(), f, ensure_ascii=False, indent=2)
        except OSError as e:
            logger.warning(f"保存活跃会话 {ctx.conversation_id} 失败: {e}")

    def _load_to_memory(self, conversation_id: str) -> ConversationContext:
        ...
        if conversation_id in self._sessions:
            ...
            if self._check_expired(ctx) or self._check_idle(ctx):
                ...
                return self._create_new(conversation_id)
            if self._redis_store:
                self._redis_store.refresh_ttl(conversation_id, self._idle_timeout)
            return ctx

        # 1. Redis
        if self._redis_store:
            ctx = self._redis_store.load(conversation_id)
            if ctx is not None:
                if self._check_expired(ctx) or self._check_idle(ctx):
                    self._archive_session(ctx)
                    return self._create_new(conversation_id)
                ...
                logger.info(f"从 Redis 恢复会话 {conversation_id} ({ctx.total_turns} 轮)")
                return ctx

        # 2. 活跃目录
        active_path = self._active_dir() / f"{conversation_id}.json"
        if active_path.exists():
            try:
                ...
                ctx.status = "active"
                self._sessions[conversation_id] = ctx
                logger.info(f"从活跃目录恢复会话 {conversation_id} ({ctx.total_turns} 轮)")
                return ctx
            except Exception as e:
                logger.warning(f"从活跃目录加载会话 {conversation_id} 失败: {e}")

        # 3. 冷层归档（兜底）
        restored = self.restore_from_archive(conversation_id)
        if restored is not None:
            return restored

        # 新建
        ctx = self._create_new(conversation_id)
        self._sessions[conversation_id] = ctx
        return ctx

    def _create_new(self, conversation_id: str) -> ConversationContext:
        """创建新会话（状态: created）"""
        ctx = ConversationContext.create_new(conversation_id)
        ctx.status = "created"
        _audit.log_session_create(conversation_id)
        return ctx
    ...
    def _archive_session(self, ctx: ConversationContext) -> None:
        ...
        try:
            ...
            path = archive_dir / f"{ctx.conversation_id}.json"
            with open(path, "w", encoding="utf-8") as f:
                json.dump(archived.to_dict(), f, ensure_ascii=False, indent=2)
            if self._redis_store:
                self._redis_store.delete(ctx.conversation_id)
            _audit.log_archive(ctx.conversation_id, ctx.total_turns, duration)
            logger.info(f"会话 {ctx.conversation_id} 已自动归档（{ctx.total_turns} 轮）")
        except OSError as e:
            logger.warning(f"归档会话 {ctx.conversation_id} 失败: {e}")
    ...
    def get_context(self, conversation_id: str) -> list[dict]:
        ...
        if not conversation_id:
            return []
        ctx = self._load_to_memory(conversation_id)
        if ctx is None:
            return []
    ...
    def get_conversation_context(self, conversation_id: str) -> ConversationContext | None:
        ...
        if not conversation_id:
            return None
        return self._load_to_memory(conversation_id)

    def update(self, conversation_id: str, query: str, messages: list[dict]) -> None:
        """存储一轮对话的新增消息
        ...
        if self._check_expired(ctx):
            self._archive_session(ctx)
            self._sessions.pop(conversation_id, None)
            logger.info(f"会话 {conversation_id} 已超最大存活时间，自动归档")
            return
        ...
        logger.info(
            f"Session {conversation_id}: 第 {ctx.total_turns} 轮已存储 "
    ...
    def clear(self, conversation_id: Optional[str] = None) -> None:
        ...
        if conversation_id:
            self._sessions.pop(conversation_id, None)
            if self._redis_store:
                self._redis_store.delete(conversation_id)
            logger.info(f"Session {conversation_id} 已清除")
        else:
    ...
    def add_warm_summary(
        self, conversation_id: str, summary: "TopicSnapshot"
    ) -> None:
        """添加温层摘要"""
        if not conversation_id:
            return
    ...
    def set_current_topic(
        self, conversation_id: str, topic: Optional["TopicSnapshot"]
    ) -> None:
        """设置当前话题"""
        if not conversation_id:
            return
    ...
    def archive(self, conversation_id: str, user_id: str = "") -> Optional["ArchivedSession"]:
        """关闭并归档会话（冷层持久化）"""
        if not conversation_id:
            return None
        ctx = self._load_to_memory(conversation_id)
        ...
        archive_dir = self._archive_dir()
        archive_dir.mkdir(parents=True, exist_ok=True)
        archive_path = archive_dir / f"{ctx.conversation_id}.json"
        try:
            with open(archive_path, "w", encoding="utf-8") as f:
                json.dump(archived.to_dict(), f, ensure_ascii=False, indent=2)
        except OSError as e:
            logger.warning(f"归档会话 {conversation_id} 失败: {e}")
        ...
        ctx.status = "archived"
        logger.info(f"Session {conversation_id} 已归档 ({archived.total_turns} 轮)")
        return archived

    def restore_from_archive(self, conversation_id: str) -> ConversationContext | None:
        ...
        """
        archive_path = self._archive_dir() / f"{conversation_id}.json"
        active_path = self._active_dir() / f"{conversation_id}.json"
        ...
        if archive_path.exists():
            try:
                ...
                self._sessions[conversation_id] = ctx
                logger.info(f"会话 {conversation_id} 已从冷层恢复 ({ctx.total_turns} 轮)")
                return ctx
            except Exception as e:
                logger.warning(f"从冷层恢复会话 {conversation_id} 失败: {e}")
                return None
        ...
        if active_path.exists():
            try:
                ...
                self._persist_restored(ctx)
                self._sessions[conversation_id] = ctx
                logger.info(f"会话 {conversation_id} 已从活跃目录恢复 ({ctx.total_turns} 轮)")
                return ctx
            except Exception as e:
                logger.warning(f"从活跃目录恢复会话 {conversation_id} 失败: {e}")
                return None

        logger.warning(f"会话 {conversation_id} 在冷层和活跃目录均不存在")
        return None
    ...
    def _auto_archive_file(self, path: Path, data: dict) -> None:
        ...
        try:
            ...
            archive_dir = self._archive_dir()
            archive_dir.mkdir(parents=True, exist_ok=True)
            archive_path = archive_dir / f"{conversation_id}.json"
            with open(archive_path, "w", encoding="utf-8") as f:
                json.dump(archived.to_dict(), f, ensure_ascii=False, indent=2)
            path.unlink()  # 删除活跃目录中的文件
            logger.info(f"会话 {conversation_id} 已自动归档（空闲超时）")
        except Exception as e:
    ...
    def get_session_status(self, conversation_id: str) -> str:
        """获取会话状态"""
        if conversation_id in self._sessions:
            return self._sessions[conversation_id].status
        if self._redis_store and self._redis_store.exists(conversation_id):
            return "active"
        if (self._archive_dir() / f"{conversation_id}.json").exists():
            return "archived"
        return "unknown"
